...
<script>
	function searchBtn() {
		...
		// 검색 초기화
	    var PRODUCTION_EQUIPMENT_CODE = "";
	    var PRODUCTION_DEFECT_CODE = "";
	    if (document.getElementById('PRODUCTION_EQUIPMENT_INFO_NAME').value == "") {
	    	if(document.getElementById('PRODUCTION_EQUIPMENT_CODE').value != "")
	    	  document.getElementById('PRODUCTION_EQUIPMENT_CODE').value = PRODUCTION_EQUIPMENT_CODE
	    }
	    if (document.getElementById('PRODUCTION_DEFECT_NAME').value == "") {
	    	if(document.getElementById('PRODUCTION_DEFECT_CODE').value != "")
	    	  document.getElementById('PRODUCTION_DEFECT_CODE').value = PRODUCTION_DEFECT_CODE
	    }
		...
		data = {
			...
			PRODUCTION_EQUIPMENT_CODE : document.getElementById('PRODUCTION_EQUIPMENT_CODE').value,
			PRODUCTION_EQUIPMENT_INFO_NAME : document.getElementById('PRODUCTION_EQUIPMENT_INFO_NAME').value,		
			PRODUCTION_DEFECT_CODE : document.getElementById('PRODUCTION_DEFECT_CODE').value,
			PRODUCTION_DEFECT_NAME : document.getElementById('PRODUCTION_DEFECT_NAME').value,
		}

		$.ajax({
			method : "GET",
			url : "${contextPath}/InspectionRest/InspectionEquipment?data="
	...
	function PRODUCTION_EQUIPMENT_CODE_SELECT() {
		// 웹 브라우저내에 PRODUCTION_EQUIPMENT_INFO_NAME변수를 생성해서 PRODUCT_ITEM_NAME이라는 ID값의 Input태그에 담긴 값을 임시적으로 저장한다.
		localStorage.setItem('PRODUCTION_EQUIPMENT_INFO_NAME', document
				.getElementById("PRODUCTION_EQUIPMENT_INFO_NAME").value);
		var url = "${contextPath}/EquipmentQuality_ItemCode";
		var name = "PRODUCTION_EQUIPMENT_CODE";
		var option = "width = 500, height = 500, top = 100, left = 200, location = no"
		openWin = window.open(url, name, option);
	}

	function PRODUCTION_DEFECT_CODE_SELECT() {
		// 웹 브라우저내에 PRODUCTION_DEFECT_NAME변수를 생성해서 PRODUCT_ITEM_NAME이라는 ID값의 Input태그에 담긴 값을 임시적으로 저장한다.
	...
	function tableSetting(datas) {
			...
			rowFormatter : function(row) {
				if (row.getData().production_SERIAL_NUM == "Sub Total") {
			...
			{title : "순번", field : "production_INFO_NUM", headerHozAlign : "center", hozAlign : "left"}, 
						...
						<table
							...
							<tr>
								<td colspan="25"><font style="margin-left: 5px;"
									size="5px;" face="Noto Sans KR">검사 이력 관리 (설비별)</font></td>
							</tr>
							<tr>
								<td colspan="6"></td>
							...
							<tr>
								<td colspan="3"></td>
								<td colspan="2" align="right">설비코드&nbsp;</td>
								<td colspan="3" align="left"><input
									id="PRODUCTION_EQUIPMENT_CODE" type="text" style="width: 90%;"
									autocomplete="off" disabled="disabled"></td>
								<td colspan="1" align="right">설비명&nbsp;</td>
								<td colspan="3" align="left"><input
									id="PRODUCTION_EQUIPMENT_INFO_NAME" type="text"
									style="width: 90%;" autocomplete="off"
									onkeypress="javascript:if(event.keyCode==13) {PRODUCTION_EQUIPMENT_CODE_SELECT()}">
								</td>
							</tr>
							<tr>
								<td colspan="3"></td>
								<td colspan="2" align="right">불량코드&nbsp;</td>
								<td colspan="3" align="left"><input
									id="PRODUCTION_DEFECT_CODE" type="text" style="width: 90%;"
									autocomplete="off" disabled="disabled"></td>
								<td colspan="1" align="right">불량명&nbsp;</td>
								<td colspan="3" align="left"><input
									id="PRODUCTION_DEFECT_NAME" type="text" style="width: 90%;"
									autocomplete="off"
									onkeypress="javascript:if(event.keyCode==13) {PRODUCTION_DEFECT_CODE_SELECT()}">
								</td>
							</tr>
						</table>
