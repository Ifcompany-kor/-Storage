...
<script>
	//검색
	function searchBtn() {
		if (document.getElementById('startDate').value.length < 5) {
			alert("시작일은 반드시 입력하여 주십시오.");
			return;
		}

		if (document.getElementById('endDate').value.length < 5) {
		...
		$.ajax({
			method : "GET",
			url : "${contextPath}/PerformanceRest/MonthProduct?data="+ encodeURI(JSON.stringify(data)) + "",
	...
	function tableSetting(datas) {
		table = new Tabulator("#example-table", { 
			...
			rowFormatter: function(row){
	    		if(row.getData().production_PRODUCT_CODE == "Sub Total"){
	    	...
	    	height:"75%",
     		...
     		//{title:"번호", field:"production_INFO_NUM", headerHozAlign:"center",hozAlign:"center"},
     		//시리얼 번호 행은 sub total, grand total 때문에 가운데정렬
     		//{title:"시리얼번호", field:"production_SERIAL_NUM", headerHozAlign:"center",hozAlign:"center"},
	     	...
	     	{title:"월", field:"production_MODIFY_D", headerHozAlign:"center",hozAlign:"right"},
	     	{title:"생산 합계", field:"production_P_Qty", headerHozAlign:"center",hozAlign:"right"},
	     	{title:"불량 합계", field:"production_B_Qty", headerHozAlign:"center",hozAlign:"right"},
	     	{title:"양품 수량", field:"production_PRODUCTS_VOLUME", headerHozAlign:"center",hozAlign:"right"},
	     	{title:"불량율", field:"production_DEFECT_RATE", headerHozAlign:"center",hozAlign:"right"},
...
<body class="sb-nav-fixed">
						...
						<table
							...
							<tr>
								<td colspan="25"><font style="margin-left: 5px;"
									size="5px;" face="Noto Sans KR">생산 실적 관리 (월별)</font></td>
							</tr>
							<tr>
								<td colspan="6"></td>
							...
							<tr>
								...
								<td colspan="3" align="left"><input
									id="PRODUCT_ITEM_CODE" type="text" style="width: 90%;"
									autocomplete="off" disabled="disabled"></td>
								<td colspan="1" align="right">제품명&nbsp;</td>
								<td colspan="3" align="left"><input
									id="PRODUCT_ITEM_NAME" type="text" style="width: 90%;"
									autocomplete="off"
	...
	window.onload = function() {
        //startDate
        var date = new Date();
        var lastYear = new String(date.getFullYear() - 1);
        var year = new String(date.getFullYear());
        var month = new String(date.getMonth() + 1);
        var firstDayOfMonth = new Date(date.getFullYear(), date.getMonth(), 1);
        var lastMonth = new Date (firstDayOfMonth.setDate(firstDayOfMonth.getDate() -1));
        var lastMonth1 = new String(firstDayOfMonth.getMonth() + 1);
        var firstDay = new String(new Date(year, month, 1).getDate());
        var endDay = new String(new Date(lastYear, lastMonth1, 0).getDate());
        ...
        document.getElementById('firstDay').value = lastYear + lastMonth1 + firstDay;
        document.getElementById('endDay').value = year + month + endDay;
        document.getElementById('startDate').value = lastYear + "-" + lastMonth1;
